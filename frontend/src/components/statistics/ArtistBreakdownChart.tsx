...
import { SongsByArtist } from '../../features/statistics/types';
...
export const ArtistBreakdownChart: React.FC<ArtistBreakdownChartProps> = ({ data }) => {
  if (!data || data.length === 0) {
    return (
      <EmptyState
        ...
        description="Add songs to see top artist rankings."
      />
    );
  }

  // Sort descending by count and slice Top N
  const { chartData, totalArtists, hiddenCount } = useMemo(() => {
    ...
    const formatted = topItems.map((item) => ({
      name: item.artist.length > 16 ? `${item.artist.slice(0, 14)}...` : item.artist,
      fullName: item.artist,
      count: item.count,
    }));
      ...
      <Flex alignItems="center" justifyContent="space-between" pb={2} borderBottom="1px solid" borderColor="border" flexWrap="wrap" gap={1}>
        ...
        <Flex alignItems="center" gap={2}>
          <Text fontSize={0} color="textMuted" fontWeight={500}>
            {hiddenCount > 0
              ? `Top ${TOP_N} of ${totalArtists} artists (+${hiddenCount} more)`
      ...
      <div style={{ width: '100%', height: chartHeight }}>
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            />
            <Tooltip
              formatter={(value: any) => [`${value} ${Number(value) === 1 ? 'song' : 'songs'}`, 'Tracks']}
